...
# Схема проекта "Шашки". Самописная простая игра на React Create App + Tailwind
```bash
checkers/
├── .npm/
...
│   └── ...             # остальные иконки и превьюшка 
│
├── src/              
│   ├── app              # провайдер, роутер и обвертка для анимации
│   ├── components       # layouts, sections
│   ├── logic            # вся логика ходов и всего что касается шашек
│   ├── pages            # страницы
│   ├── hooks            # хуки
│   ├── store            # хук для взаимодействия с воркером
│   ├── types            # swiper.d.ts, types.ts
│   ├── workers          # вместо сервера
│   ├── index.css        # стили, импорт tailwind и конфига
...
```js
 return (
    <BrowserRouter basename="/games/checkers">
...
RewriteCond %{REQUEST_URI} !^/games/checkers/assets/
RewriteCond %{REQUEST_URI} !^/games/checkers/static/

# Всё остальное → index.html (SPA fallback)
RewriteRule ^ /games/checkers/index.html [L]
